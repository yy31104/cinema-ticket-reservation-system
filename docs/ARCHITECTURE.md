...
Cinema (1) ───< (N) Screening (1) ───< (N) Reservation (N) >─── (1) ApplicationUser
  ...
  SeatsPerRow        PosterUrl?               RowNumber                PhoneNumber
                    Synopsis?
                    DurationMinutes?
                    Genre?
                    AgeRating?
...
- `Screening` carries optional film metadata directly (`PosterUrl`, `Synopsis`,
  `DurationMinutes`, `Genre`, `AgeRating`). A separate `Film` aggregate is deferred until
  cross-screening reuse is needed.
- `Screening.StartTime` is the local wall-clock time at the cinema and is stored unconverted.
  UTC plus venue timezone modeling is deferred.
...
| Content | Film metadata is optional; seeded demo screenings/posters are deferred |
...
4. **Film metadata** — extend the schema (poster, synopsis, runtime, genre, age rating) with a
   migration; update DTOs and admin create form. Seeded demo screenings/posters are deferred
   so static showtimes do not age and the PR stays focused.
...
- **ADR-0005** Film metadata lives on `Screening`; extracting a separate `Film` entity is
  deferred until cross-screening reuse is needed. *Rationale:* keeps the first content/schema
  PR additive and small.
- **ADR-0006** `Screening.StartTime` is local wall-clock time at the cinema and is stored
  unconverted; UTC plus venue timezone modeling is deferred. *Rationale:* preserves current
  behavior while documenting the time assumption for a future scheduling PR.
